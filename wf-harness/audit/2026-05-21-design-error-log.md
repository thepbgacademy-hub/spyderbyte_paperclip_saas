...
22. The deferred approval-card foreign-key seam should be proven with a transaction-pinned Postgres repository test, not only with in-memory passthrough tests. The important proof is that the approval update and child-card insert share one leased transaction client and roll back together on failure.
