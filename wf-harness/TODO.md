...
- [x] Record the first-subscriber post-invite observation template.
  ...
  - [x] Add the pending observation template at `audit/2026-06-30/first-subscriber-post-invite-observation-template.json`.
  - [x] Add the operator-facing post-invite runbook at `deploy/runbooks/first-subscriber-post-invite-runbook.md`.
  - [x] Keep `subscriberInviteSent: false` and `verdict: "pending"` until the operator performs the invite out of band.
  - [x] Do not turn this observation template into onboarding UI, rollback automation, scheduler automation, export replay, workflow/package expansion, or `api.spyderbyte.cloud` cutover.
  - [x] Keep the phase non-destructive: no VPS, Docker, Caddy, DNS, database, runtime, worker, queue, seed, dashboard-visual, workflow-family, package-overlay, rollback, or operator mutation.
...
- [x] Close the bounded local multi-lane continuation acceptance proof for the CEO-governed native loop.
  - [x] Keep the phase local-only and in-memory: no VPS, no public dashboard/start widening, no export writer/replay changes, no queue substrate changes, no reset utilities, and no workflow-family/package-overlay expansion.
  - [x] Extend `tests/harness-e2e-run-loop.test.ts` to prove the uninterrupted two-lane path: CFO lane completes, CEO receives a `next_lane_decision`, CEO explicitly starts the CMO lane, the CMO lane claims through the native worker seam with a persisted execution claim token, final assembly is queued only after both lanes are done, and the closed completion package contains both lane outcomes.
  - [x] Close the reviewer-found governance gap with a true RED/GREEN fix: generic worker polling now stays quiet while unresolved CEO `queue_ceo_review` attention is pending, so the next approved lane cannot be claimed until the explicit reviewed dispatch path targets it.
  - [x] Add focused worker-executor regressions proving the same generic-vs-targeted dispatch seam directly: unresolved CEO next-lane review suppresses generic dispatch, while explicit `targetCardId` dispatch still claims the reviewed lane.
...
- [x] Close the bounded live dashboard result-approval read proof after the isolated stage refresh.
  - [x] Add a dry-run-first, GET-only proof script (`npm run prove:live-dashboard-result-approval`) that mints or accepts a scoped portal session and reads `/api/dashboard` without creating runs, exporting files, touching provider lanes, or mutating queue/board state.
  - [x] Add focused proof coverage for the live proof script and the missing migration-helper readiness guard.
  - [x] Refresh only `wf-stage-api` on VPS2 to the reviewed current-branch API image `spyderbyte/api:wf-stage-20260702-dashboardapproval1`; do not recreate web, worker, Caddy, DNS, Paperclip, provider credentials, or shared domains.
  - [x] Apply only the missing idempotent schema migration `0034_wf_harness_result_approval_states.sql` on the isolated stage database after the refreshed API proved the resolver path was active but the table was absent.
  - [x] Verify the live GET-only proof against `https://wf-api.spyderbyte.cloud`: HTTP 200, `resultApprovalStates` present as an object, sanitized workflows only, and no raw prompts/logs/secrets in proof output.
  - [x] Complete the remaining live-testing readiness gate by proving the public dashboard journey in browser harness against the refreshed Wealth Factory lane and then running the primary launch-lane stage-stability suite.
- [x] Close the first launch-lane live-testing readiness gate for the refreshed Wealth Factory stage lane.
  - [x] Add `scripts/prove-live-dashboard-browser-journey.mjs` and `npm run prove:live-dashboard-browser-journey` as a dry-run-first browser-harness proof that mints or accepts a scoped portal session, drives the public board page through CDP, verifies the shell/board/results views, checks result-approval localStorage shape, and rejects forbidden social-media drift text without POSTing or mutating workflow state.
  - [x] Keep the result-approval read bridge GET-only and schema-gated through `scripts/prove-live-dashboard-result-approval.mjs` and `0034_wf_harness_result_approval_states.sql`.
  - [x] Gate Codex auth-home readiness only on the Codex-subscription launch workflow `wf_connect_first_workflow`; do not pass primary Codex readiness artifacts to non-Codex domain-pack lanes.
  - [x] Propagate explicit `--lanes primary` through `prove:stage-live`, `prove:stage-live-native-execution`, fairness, and soak so the launch gate exercises the intended first-client lane instead of silently widening into unbuilt families.
  - [x] Forward only public API/portal origins into remote fairness and soak proof containers, and never forward API session signing keys or provider secrets through proof commands.
  - [x] Use progress-mode pressure for the primary launch lane because `wf_connect_first_workflow` is intentionally attention-gated and can truthfully block for user review; preserve drain-mode expectations for broader future family gates.
  - [x] Verify with focused tests, server/web builds, live GET-only result-approval proof, browser-harness dashboard journey proof, and `npm run prove:stage-stability -- --lanes primary` returning `phase: "stage_stability_complete"`.
  - [x] Store sanitized launch-readiness evidence in `audit/2026-07-02/stage-stability-primary-launch-readiness-summary.json`; keep raw soak/log/jsonl captures local-only because they include live stage topology and run identifiers.
  - [x] During human/operator live testing, capture the first real dashboard blocker loop: a generic CFO pricing-lane unblock moved the card out of blocked state but did not provide the pricing assumptions, target customer, current offer, competitor anchors, or margin constraints needed for execution.
  - [x] Tighten the unblock contract so blocked lanes require a substantive tenant-safe unblock summary before they can re-enter execution; deploy the isolated stage API/worker refresh and verify live board state exposes `resumeSummary.required = true` for `await_unblock`.
  - [ ] Next bounded slice: continue human/operator live testing on `wf_connect_first_workflow` using a sample-company pricing input, then separately schedule matched domain-specific prompt/data pack acceptance for `wf_tax_strategy` and package follow-up after those packs exist.
