...
- The successful live proof used the explicit readiness-gate inputs `--api-codex-home-readiness-proof`, `--worker-codex-home-readiness-proof`, and `--codex-auth-state-ref`; future live proofs must keep those target-matched instead of trusting container health alone.
- Saved evidence:
...
- Important scope note: the wider stage wrapper correctly failed closed when it attempted `wf_tax_strategy` with connect-first readiness artifacts. That is not a connect-first failure; each workflow family requires matching tenant/workflow/auth-state readiness artifacts before live proof.
- Current continuation point:
  - commit/push the reauth and connect-first proof evidence after focused verification and GitNexus review
  - next launch-facing phase should prove the browser/client journey on `wf_connect_first_workflow` now that the native engine round trip is green
  - do not widen into `wf_tax_strategy`, package follow-up, export, scheduler, DNS/Caddy, Paperclip state, or shared-host cutover unless that is chosen as a separate bounded phase with matching readiness artifacts

- Deployed the bounded worker-side Codex execution wiring fix to the isolated Wealth Factory stage lane only.
- Local TDD guard now requires `wf-stage-worker` to receive the tenant-isolated `CODEX_HOME` mount and requires `Dockerfile.worker` to expose `/app/node_modules/.bin` on `PATH`.
- Built and deployed only `spyderbyte/worker:wf-stage-20260701-workercodexhome2`; `wf-stage-api` remained on `spyderbyte/api:wf-stage-20260701-codexresolver1` and `wf-stage-web` remained unchanged.
- Live worker checks now pass the container-wiring layer: worker is healthy, `CODEX_HOME` exists and is writable, and the packaged Codex CLI is present in the image.
- Added sanitized smoke-error propagation to `npm run prove:codex-auth-home-readiness` so Codex auth failures no longer collapse into an opaque `codex_smoke_failed`.
- Fresh live readiness proofs against both `wf-stage-api` and `wf-stage-worker` now fail for the same reason: `refresh_token_invalidated` / "Your session has ended. Please log in again."
- Current launch blocker: the tenant-isolated OpenAI Codex device-login state on VPS 2 is revoked. Stop retrying native workflow execution until the Codex home is re-authenticated out of band, then rerun readiness against both containers and rerun the bounded live native proof.
- The readiness proof now classifies this condition as `codex_auth_session_revoked` instead of leaving it as an opaque `codex_smoke_failed`, so future operator runs have a direct stop/re-authenticate signal.
- Added/updated evidence:
  - `audit/2026-07-01/codex-auth-home-readiness-api-auth-revoked.json`
  - `audit/2026-07-01/codex-auth-home-readiness-worker-auth-revoked.json`
  - `audit/2026-07-01/codex-auth-home-readiness-api-auth-session-revoked.json`
  - `audit/2026-07-01/codex-auth-home-readiness-worker-auth-session-revoked.json`
  - `audit/2026-07-01/codex-auth-home-readiness-worker-after-worker-mount.json`
  - `audit/2026-07-01/live-native-execution-after-resolver-deploy.json`
- No DNS, Caddy, web image, shared `api.spyderbyte.cloud`, database/package requirements, Paperclip state, export, scheduler, onboarding UI, or tenant-wide workflow rows were changed in this phase.
- Next continuation point:
  - re-authenticate the tenant-isolated Codex device login in `[redacted-codex-home]` without printing or committing credential material
  - rerun `npm run prove:codex-auth-home-readiness` against `wf-stage-api` and `wf-stage-worker` with `--target-tenant`, `--target-workflow wf_connect_first_workflow`, and `--auth-state-ref`; if either returns `codex_auth_session_revoked`, stop and refresh the device login again
  - after both return `codex_auth_home_ready`, rerun the bounded live native execution proof for `wf_connect_first_workflow` with both `--api-codex-home-readiness-proof` and `--worker-codex-home-readiness-proof`, plus `--codex-auth-state-ref` unless the operator env already sets `WF_OPENAI_CODEX_AUTH_STATE_REF`; the proof now fails closed before remote reservation or advancement if either artifact is missing, revoked, mismatched, mutation-tainted, pointed at the wrong auth-state reference, or tied to a different `CODEX_HOME` fingerprint

- Added a fail-closed Codex auth-home readiness artifact gate in front of live native execution proof.
- The gate validates both API and worker readiness artifacts for `codex_auth_home_ready`, no mutation, matching tenant/workflow/auth-state reference, matching `CODEX_HOME` fingerprint, and the expected stage containers before allowing the live native execution proof to reserve or advance any run.
- This converts the current launch-blocker guidance into executable proof-script behavior: while the latest artifacts remain `codex_auth_session_revoked`, live native execution is blocked by design.
- Added/updated:
  - `scripts/lib/codex-readiness-proof-gate.mjs`
  ...
  - `tests/codex-readiness-proof-gate.test.ts`
  - `tests/live-native-execution-script.test.ts`
...
- Added dry-run-first operator command `npm run repair:openai-device-provider-binding` to plan the OpenAI device provider binding repair without mutating live state unless explicit execute gates are supplied.
- Hardened the repair command so execute mode requires `codexHome`, `authStateRef`, `--confirm-codex-home-ready`, a target-matched `--codex-home-readiness-proof` with a matching CODEX_HOME fingerprint, and an operator-supplied stage DB URL before any transaction can run.
- Implemented the target-scoped transaction contract for the controlled first-subscriber lane: update the workflow template provider binding by tenant/workflow id, verify the `wfpc.package_provider_requirements` seam by package id without mutating it, revoke only active `openai_chatgpt_codex_subscription` references for the target tenant, and upsert a metadata-only `wfpc.secret_references` row. The repair lane must not mutate existing `workflow_runs`, Paperclip state, shared-host routing, `wfpc_private.vault_secrets`, or BYOK/API-provider lanes.
- Live stage note: the public board/proof workflow id remains `wf_connect_first_workflow`, but the actual `wfpc.workflow_templates.id` row is the UUID `44444444-4444-4444-8444-444444444444`; pass that UUID as `--workflow-template` for live repair.
- Safety note: the revoke step is tenant/provider scoped because `secret_references` has no workflow column. It is acceptable only for the current controlled first-subscriber single-workflow lane and must not be generalized to multi-workflow tenants without a schema/design change.
...
- Added the stage compose/env contract for `WF_OPENAI_CODEX_HOME` on the API lane only, mounted it into `wf-stage-api` as `CODEX_HOME`, and hardened the readiness proof so a present but unwritable auth home reports `codex_home_not_writable` before any smoke prompt or provider repair.
- Provisioned the tenant-isolated VPS Codex home, refreshed only `wf-stage-api`, and reran `npm run prove:codex-auth-home-readiness -- --execute`. Result advanced to `codex_smoke_failed`: Codex CLI is present, `CODEX_HOME` exists and is writable, but authenticated Codex device state is not yet installed in that VPS auth home.
- Installed the operator-approved Codex auth state in the tenant-isolated VPS `CODEX_HOME` without printing or committing credential material, then reran `npm run prove:codex-auth-home-readiness -- --execute`. Result: `codex_auth_home_ready`; the non-secret smoke prompt passed and no DB/workflow/DNS/Caddy/provider/Paperclip mutation was performed.
- Ran the hotzone secret scan over the changed Codex auth-home readiness files and targeted hotzone git history. Result: no confirmed exposed secrets; only compose placeholders, sanitizer regexes, and prose/test markers were found. Report recorded at `audit/2026-06-30/secret-scan-report.md`.
- Hardened `npm run repair:openai-device-provider-binding` so execute mode requires a `--codex-home-readiness-proof` artifact and validates `codex_auth_home_ready`, Codex CLI, writable `CODEX_HOME`, smoke prompt success, no prior mutation, matching tenant/workflow/auth-state/CODEX_HOME fingerprint, and a DB URL before the target-scoped transaction can run.
- Decision: the Codex auth-home readiness hotzone is safe to proceed from a repository secret-exposure standpoint; keep actual VPS Codex auth state out of source control, audit artifacts, logs, and summaries.
- Added:
...
- Approved VPS operator changes in this lane were limited to Docker group access, `wf-stage-api` image/container refresh, tenant-isolated Codex home provisioning, and Codex auth-state installation. No Caddy, DNS, database, worker, scheduler, export, onboarding-UI, workflow/package, rollback, Paperclip, BYOK/API-provider lane, or cutover mutation was performed.
- Next continuation point:
  - deploy the local runtime resolver fix for `openai_chatgpt_codex_subscription` to the isolated Wealth Factory stage image, then rerun the same live native proof
  - do not keep retrying the existing live container image and expect the device-auth lane to pass; the local fix is in `src/providers/runtime-provider-execution.ts` / `.js` and must be present in `wf-stage-api`
  - the controlled provider binding is repaired and the existing proof run has been rebound; evidence lives in `audit/2026-07-01/openai-device-provider-binding-db-postflight.json`, `audit/2026-07-01/openai-device-existing-proof-run-rebind.json`, and `audit/2026-07-01/live-native-execution-after-openai-device-rebound-run.json`
  - reviewer correction: do not mutate `wfpc.package_provider_requirements` for this repair lane because it is package-scoped and the live package is shared by another enabled workflow template; live restore evidence is `audit/2026-07-01/package-provider-requirement-blast-radius-restore.json`
  - the latest live proof no longer uses the old `openai_api` binding, but the still-deployed runtime image blocks on `bound provider secret was unavailable at execution time`; that is the deployment/runtime resolver gap to close next
...
- The matched isolated-stage live proof now behaves truthfully on the current code:
  - reran `node scripts/prove-live-harness-export.mjs --env-file [redacted-local-secrets-path] --ssh-env-file [redacted-local-secrets-path] --ssh-target deploy@[masked] --preflight-container wf-stage-api --tenant 22222222-2222-4222-8222-444444444444 --user 11111111-1111-4111-8111-333333333333 --workflow 44444444-4444-4444-8444-666666666666 --board-workflow wf_tax_strategy --delivery-mode auto --timeout-ms 60000`
...
- Focused local verification is green for the bounded seam:
  ...
  - `npm run build`
  - `npm run build:server`
- The isolated stage live gate is also green for the same bounded lane. On June 28, 2026 the connect-first proof passed against `deploy@[masked]` / `wf-stage-api`, proving `direct_public_reservation_verified`, `native_blocked_reached`, `native_attention_resolved`, and `round_trip_verified` on durable run `eb420710-c786-4801-b54d-5c3adb39f0fc`.
...
- The bounded local verification for that slice is green:
  ...
  - `npm run build`
  - `npm run build:server`
- The isolated live stage proof is also green for the new seam. On June 28, 2026 `node scripts/prove-live-native-execution.mjs --env-file [redacted-local-secrets-path] --ssh-env-file [redacted-local-secrets-path] --ssh-target deploy@[masked] --preflight-container wf-stage-api --tenant 22222222-2222-4222-8222-222222222222 --user 11111111-1111-4111-8111-111111111111 --workflow wf_connect_first_workflow --workflow-template 44444444-4444-4444-8444-444444444444 --timeout-ms 60000 --fresh-run true` returned the structured conflict result with durable run `eb420710-c786-4801-b54d-5c3adb39f0fc` instead of the raw uniqueness-index failure.
...
- That proof used the real stage env from `[redacted-local-secrets-path]`, resolved each tenant's live `workflow_templates.id` UUID inside `wf-stage-api` for preflight, and kept the browser-facing shell/API checks on the public workflow ids so the runtime and public identity seams both stayed truthful.
...
- Rebuilt the board pulse and top metrics around launch-critical posture only, while preserving the live/preview distinction, contract-owned board actions, CEO approval review surfaces, and the drawerâ€™s bounded continuity detail.
...
- Distinguish ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œstale actionÃƒÂ¢Ã¢â€šÂ¬Ã‚Â from ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œstale payload.ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â If the current board still exposes the same bounded action but the old payload no longer fits the current request-field contract, suppress replay, keep reset available, and tell the operator the contract changed instead of treating both cases as the same kind of stale failure.
- Do not collapse every `invalid_request` into ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œreset the composer.ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â If the current board still exposes the same action and the old payload still fits the bounded contract, keep recovery on reload/dismiss instead of pretending the request drifted when the real cause may be elsewhere.
...
- When that contract-refresh guidance is shown, it should identify the affected bounded actions instead of saying only that ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œsomething was pruned.ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Concurrency-safe recovery depends on telling the operator which action path or composer moved, not just that the page cleaned up local state.
...
- Deepened `followThroughItems` from ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œimplemented action happenedÃƒÂ¢Ã¢â€šÂ¬Ã‚Â into ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œimplemented action plus why it was allowedÃƒÂ¢Ã¢â€šÂ¬Ã‚Â by carrying bounded policy reason, resolution label, and recommendation/objective summaries from the decision ledger. That keeps suggested-versus-implemented history exportable later without asking the UI or a future Obsidian sync job to reconstruct intent from scattered decision rows.
...
- Tightened the outbox reconciliation seam too. If enqueue succeeds or BullMQ reports `already_queued` but the local `markWorkflowRunQueued(...)` step loses its claim, the outbox worker now performs a bounded repository confirmation before falling back to retry, so ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œexternal enqueue already happenedÃƒÂ¢Ã¢â€šÂ¬Ã‚Â stops drifting back into the same bucket as a real enqueue failure.
...
- Started the next bounded isolated-stage replay-cycle proof slice on June 28, 2026 and closed the proof-harness mismatches, but not the live product precondition yet:
  ...
  - the matched isolated-stage replay-cycle rerun now reaches the real acceptance seam instead of dying inside the script:
    - command used:
      - `node scripts/prove-live-harness-export-replay-cycle.mjs --env-file [redacted-local-secrets-path] --ssh-env-file [redacted-local-secrets-path] --ssh-target deploy@[masked] --preflight-container wf-stage-api --tenant 22222222-2222-4222-8222-444444444444 --user 11111111-1111-4111-8111-333333333333 --workflow 44444444-4444-4444-8444-666666666666 --board-workflow wf_tax_strategy --run cdc1d911-e9a8-4810-b9f8-40476bc008a9 --timeout-ms 60000 --poll-interval-ms 1000 --max-attempts 60`
  ...
  - the bounded operator-only alternative was then proven live on the same isolated run:
    ...
    - reran the existing public replay proof unchanged:
      - `node scripts/prove-live-harness-export.mjs --env-file [redacted-local-secrets-path] --ssh-env-file [redacted-local-secrets-path] --ssh-target deploy@[masked] --preflight-container wf-stage-api --tenant 22222222-2222-4222-8222-444444444444 --user 11111111-1111-4111-8111-333333333333 --workflow 44444444-4444-4444-8444-666666666666 --run cdc1d911-e9a8-4810-b9f8-40476bc008a9 --board-workflow wf_tax_strategy --mode replay --timeout-ms 60000 --poll-interval-ms 1000 --max-attempts 60`
