...
- decide which runtime-side consumers should actually act on `queue_ceo_review`, `await_lane_resume`, and `await_unblock` beyond the current bounded event/hook seam, so the engine keeps deepening without widening into swarm behavior
