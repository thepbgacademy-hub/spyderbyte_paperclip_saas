...
- Run reconciliation is intentionally deterministic but still conservative. The harness can now derive `active`, `waiting`, `blocked`, and `assembling` from child-card/proposal state, but it does not auto-mark a run `done` yet. Leave final assembly/completion as a later explicit seam instead of guessing from child-card movement alone.
...
- Outcome summaries are intentionally no longer accepted through the advancement URL query string. Keep tenant business summaries off URL surfaces until the dashboard HTTP contract grows a parsed request body for this mutation.
