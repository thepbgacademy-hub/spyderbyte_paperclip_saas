...
describe("harness proposal resolution migration", () => {
  ...
  it("is included in the live migration helper", () => {
    ...
    expect(helper).toMatch(/0017_wf_harness_board_memory\.sql/i);
    ...
    expect(helper).toMatch(/harness_board_decisions_policy_reason_check/i);
    expect(helper).toMatch(/created_new_lane/i);
    expect(helper).toMatch(/reused_existing_lane/i);
    expect(helper).toMatch(/lane_cap/i);
    expect(helper).toMatch(/scope_guardrail/i);
  });
});
