...
describe("harness board service", () => {
  ...
  it("consults a tenant-scoped workflow registry resolver without promoting installed overlays onto the board by default", async () => {
    const repository = createInMemoryHarnessRepository();
  ...
  it("allows an explicitly board-exposed installed-package overlay when tenant install context resolves it", async () => {
    const repository = createInMemoryHarnessRepository();
    const resolveWorkflowRegistry = vi.fn().mockResolvedValue(
      createHarnessWorkflowRegistry({
        harnessEnabledWorkflowIds: ["wf-example-audit"],
  ...
  it("allows overlay board mutations through the tenant-scoped resolver when the installed package explicitly opts in", async () => {
    const repository = createInMemoryHarnessRepository();
    const resolveWorkflowRegistry = vi.fn().mockResolvedValue(
      createHarnessWorkflowRegistry({
        harnessEnabledWorkflowIds: ["wf-example-audit"],
  ...
  it("fails closed when explicit lane-resume attention reuses a stale action token", async () => {
    ...
    const waitingBoard = await service.listBoardState({ authorization: "Bearer valid" });
    const eventsBefore = await repository.listEventsForCard(created.cardId);
    ...
    const persistedCard = await repository.getCard(created.cardId);
    const eventsAfter = await repository.listEventsForCard(created.cardId);
    expect(persistedCard?.state).toBe("waiting");
    expect(eventsAfter).toEqual(eventsBefore);
  });

  it("fails closed when explicit lane-unblock attention reuses a stale action token", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    const blockedBoard = await service.listBoardState({ authorization: "Bearer valid" });
    const eventsBefore = await repository.listEventsForCard(created.cardId);
  ...
  it("fails closed when the pending attention target changes during explicit lane resolution", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    const waitingBoard = await service.listBoardState({ authorization: "Bearer valid" });
    const eventsBefore = await repository.listEventsForCard(created.cardId);
    repository.transitionCardState = async () => null;
  ...
  it("keeps generic CEO-review requested history explicit when reason metadata is absent", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_attention_requested_generic_ceo_review",
      cardId: ceoCard!.id,
      eventKind: "attention_requested",
      payload: {
        actionKind: "queue_ceo_review"
      },
      createdAt: "2026-06-03T10:12:32.000Z"
    });
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "Board attention is now waiting on CEO review."
        })
      ])
    );
  });
  ...
  it("keeps governance-hold attention-resolved history explicit without a persisted snapshot", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await service.listBoardState({ authorization: "Bearer valid" });
    const ceoCard = (await service.listBoardState({ authorization: "Bearer valid" })).cards.find(
      (card) => card.persona === "CEO"
    );
    expect(ceoCard).toBeDefined();

    await repository.insertEvent({
      id: "event_attention_resolved_governance_hold",
      cardId: ceoCard!.id,
      eventKind: "attention_resolved",
      payload: {
        actionKind: "queue_ceo_review",
    ...
    const hydrated = (await service.listBoardState({ authorization: "Bearer valid" })).cards.find(
      (card) => card.id === ceoCard!.id
    );
    ...
    expect(hydrated?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "Board attention no longer needs CEO governance-hold review."
        })
      ])
    );
  });

  it("keeps lane-resume attention-resolved history explicit without a persisted snapshot", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_attention_resolved_resume",
      cardId: created.cardId,
      eventKind: "attention_resolved",
      payload: {
        actionKind: "await_lane_resume",
      ...
      createdAt: "2026-06-03T11:11:00.000Z"
    });
  ...
  it("keeps generic CEO-review resolved history explicit when reason metadata is absent", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_attention_resolved_generic_ceo_review",
      cardId: ceoCard!.id,
      eventKind: "attention_resolved",
      payload: {
        actionKind: "queue_ceo_review"
      },
      createdAt: "2026-06-03T10:13:12.000Z"
    });
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "Board attention no longer needs CEO review."
        })
      ])
    );
  });

  it("keeps sub-card proposal history explicit when a legacy proposal event omits richer metadata", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_subcard_proposed_generic_fallback",
      cardId: created.cardId,
      eventKind: "subcard_proposed",
      payload: {},
      createdAt: "2026-06-04T09:14:20.000Z"
    });
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "A supporting sub-card was proposed for CEO review."
        })
      ])
    );
  });

  it("keeps proposal-absorbed history explicit when a legacy absorb event omits richer metadata", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_proposal_absorbed_generic_fallback",
      cardId: created.cardId,
      eventKind: "proposal_absorbed",
      payload: {},
      createdAt: "2026-06-04T09:14:25.000Z"
    });
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "The CEO folded a supporting request into this active lane."
        })
      ])
    );
  });
  ...
  it("keeps created-card history explicit when the legacy event omits title metadata", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_created_generic_fallback",
      cardId: created.cardId,
      eventKind: "created",
      payload: {},
      createdAt: "2026-06-03T10:13:30.000Z"
    });
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "Card was opened for this persona lane."
        })
      ])
    );
  });

  it("keeps lane-status history explicit when a legacy state-change event omits state metadata", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_state_changed_generic_fallback",
      cardId: created.cardId,
      eventKind: "state_changed",
      payload: {},
      createdAt: "2026-06-03T10:13:45.000Z"
    });
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "Lane status moved to Updated."
        })
      ])
    );
  });

  it("keeps generic progress-note history explicit when a legacy comment event omits message metadata", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_comment_added_generic_fallback",
      cardId: created.cardId,
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "A new progress note was added to this lane."
        })
      ])
    );
  });

  it("keeps lane-handoff history explicit when a legacy handoff event omits persona metadata", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_lane_handed_off_generic_fallback",
      cardId: created.cardId,
      eventKind: "lane_handed_off",
      payload: {},
      createdAt: "2026-06-03T10:14:15.000Z"
    });
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "CEO handed this active lane to a new persona owner."
        })
      ])
    );
  });
  ...
  it("keeps sub-card proposal history explicit when a legacy proposal event omits richer metadata", async () => {
    ...
    await repository.insertEvent({
      ...
      eventKind: "subcard_proposed",
      payload: {},
      createdAt: "2026-06-03T10:14:20.000Z"
    });
  ...
  it("keeps proposal-absorbed history explicit when a legacy absorb event omits richer metadata", async () => {
    ...
    await repository.insertEvent({
      ...
      eventKind: "proposal_absorbed",
      payload: {},
      createdAt: "2026-06-03T10:14:25.000Z"
    });
  ...
  it("keeps result-snapshot history explicit when a legacy snapshot event omits summary metadata", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_result_recorded_generic_fallback",
      cardId: created.cardId,
      eventKind: "result_recorded",
      payload: {},
      createdAt: "2026-06-03T10:14:30.000Z"
    });
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "A new outcome snapshot was recorded for this lane."
        })
      ])
    );
  });
  ...
  it("keeps generic ignored worker outcomes explicit when no reason metadata is present", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_execution_outcome_ignored_generic",
      cardId: created.cardId,
      eventKind: "execution_outcome_ignored",
      payload: {},
      createdAt: "2026-06-03T12:14:00.000Z"
    });
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "A stale or superseded worker callback was ignored."
        })
      ])
    );
  });

  it("keeps no-longer-working ignored outcomes explicit when lane-state metadata is absent", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_execution_outcome_ignored_lane_not_working_generic",
      cardId: created.cardId,
      eventKind: "execution_outcome_ignored",
      payload: {
        reason: "lane_not_working"
      },
      createdAt: "2026-06-03T12:12:30.000Z"
    });
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "A worker callback was ignored because the lane had already left active execution and was Not Working."
        })
      ])
    );
  });
  ...
  it("surfaces existing-working execution claims as resumed active-lane history", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_execution_claimed_existing_working",
      cardId: created.cardId,
      eventKind: "execution_claimed",
      payload: {
        claimKind: "existing_working_claim",
        claimedAt: "2026-06-03T10:12:00.000Z",
        previousClaimedAt: "2026-06-03T10:12:00.000Z"
      },
      createdAt: "2026-06-03T10:12:00.000Z"
    });
    await repository.insertEvent({
      id: "event_execution_start_ready_existing_working",
      cardId: created.cardId,
      ...
      createdAt: "2026-06-03T10:12:01.000Z"
    });
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "A worker resumed this lane from an already-active execution claim."
        }),
        expect.objectContaining({
          label: "Worker execution start was re-prepared for this already-claimed active lane."
        })
      ])
    );
  });
  ...
  it("surfaces execution-start-ready history as bounded board activity", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_execution_start_ready",
      cardId: created.cardId,
      eventKind: "execution_start_ready",
      payload: {
        kind: "follow_on_dispatch",
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "Worker execution start was prepared for the next lane after the prior lane outcome."
        })
      ])
    );
  });

  it("keeps follow-on execution-start-ready history stage-specific even if claim metadata survives", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_execution_start_ready_follow_on_existing_claim",
      cardId: created.cardId,
      eventKind: "execution_start_ready",
      payload: {
        kind: "follow_on_dispatch",
  ...
  it("surfaces execution-start-suppressed history as bounded board activity", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_execution_start_suppressed",
      cardId: created.cardId,
      ...
      payload: {
        ...
        failureMessage: "follow-on continuity unavailable"
      },
      createdAt: "2026-06-04T09:01:00.000Z"
    });
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "Worker execution start could not be rebuilt after the next lane was already dispatched."
        })
      ])
    );
  });
  ...
  it("surfaces replay-safe reasserted post-outcome hook failures as bounded board activity", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_execution_hook_failed_reasserted",
      cardId: created.cardId,
      eventKind: "execution_hook_failed",
      payload: {
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label:
            "A replay-safe reasserted private hook for the Queue Ceo Review post-outcome handoff failed after the worker outcome was already recorded."
        })
      ])
    );
  });
  ...
  it("keeps persona-aware follow-on dispatch history explicit when outcome-state metadata is absent", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_execution_dispatched_follow_on_persona_without_outcome",
      cardId: created.cardId,
      eventKind: "execution_dispatched",
      payload: {
        kind: "follow_on_dispatch",
      ...
      createdAt: "2026-06-03T10:10:35.000Z"
    });
  ...
  it("keeps persona-aware reactivated dispatch history explicit when outcome-state metadata is absent", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_execution_dispatched_reactivated_persona_without_outcome",
      cardId: created.cardId,
      eventKind: "execution_dispatched",
      payload: {
        kind: "follow_on_dispatch",
      ...
      createdAt: "2026-06-03T10:10:50.000Z"
    });
  ...
  it("keeps initial-lane dispatch history explicit when only execution-stage metadata survives", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_execution_dispatched_initial_stage_only",
      cardId: created.cardId,
      eventKind: "execution_dispatched",
      payload: {
        executionStage: "initial_lane_start"
      },
      createdAt: "2026-06-03T10:11:05.000Z"
    });
  ...
  it("keeps CEO review outcome summary detail explicit when no review reason is present", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_execution_outcome_committed_generic_review_with_summary",
      cardId: created.cardId,
      eventKind: "execution_outcome_committed",
      payload: {
        outcomeState: "done",
        runState: "assembling",
        postOutcomeActionKind: "queue_ceo_review",
        resultSummary: "Launch copy is ready for executive sign-off."
      },
      createdAt: "2026-06-02T15:11:20.000Z"
    });
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "A worker finished this lane and queued CEO review: Launch copy is ready for executive sign-off."
        })
      ])
    );
  });

  it("keeps CEO review outcome reason detail explicit when no summary is present", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_execution_outcome_committed_review_reason_only",
      cardId: created.cardId,
      eventKind: "execution_outcome_committed",
      payload: {
        outcomeState: "done",
      ...
      createdAt: "2026-06-02T15:11:25.000Z"
    });
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "A worker finished this lane and queued CEO review for Final Assembly."
        })
      ])
    );
  });
  ...
  it("keeps done next-lane handoff history explicit when no summary detail is present", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertEvent({
      id: "event_execution_outcome_done_dispatch_no_summary",
      cardId: created.cardId,
      eventKind: "execution_outcome_committed",
      payload: {
        outcomeState: "done",
    ...
    expect(hydratedCard?.activity).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "A worker finished this lane and handed the next lane to CMO."
        })
      ])
    );
  });
  ...
  it("does not inherit stale defer guidance once the live blocking condition has cleared", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertProposal({
      id: "proposal_persona_focus_stale_1",
      runId: board.runId,
      parentCardId: parentCard.cardId,
      requestedByCardId: parentCard.cardId,
      requestedByPersona: "cfo",
    ...
    await repository.insertDecision({
      id: "decision_persona_focus_stale_1",
      runId: board.runId,
      tenantId: "tenant_123",
      actorUserId: "user_123",
      decisionKind: "proposal_deferred",
      cardId: parentCard.cardId,
      proposalId: "proposal_persona_focus_stale_1",
      targetCardId: null,
      persona: "researcher",
      deliverableType: "forecast_model",
      policyReason: "persona_lane_cap",
    ...
    await repository.insertProposal({
      id: "proposal_persona_focus_stale_follow_on_1",
      runId: board.runId,
      parentCardId: parentCard.cardId,
      requestedByCardId: parentCard.cardId,
      requestedByPersona: "cfo",
    ...
    expect(
      proposedBoard.pendingApprovals.find((approval) => approval.id === "proposal_persona_focus_stale_follow_on_1")
  ...
  it("preserves a direct CEO request as deferred governance when child-card creation targets a done run", async () => {
    ...
    await repository.insertProposal({
      id: "proposal_done_cycle_follow_on_1",
      runId: board.runId,
      parentCardId: created.cardId,
      requestedByCardId: created.cardId,
      requestedByPersona: "cfo",
  ...
  it("keeps the real governance-history export bundle frozen after post-closure governance noise", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    await repository.insertProposal({
      id: "proposal_post_completion_export_noise_1",
      runId: board.runId,
      parentCardId: created.cardId,
      requestedByCardId: created.cardId,
      requestedByPersona: "cfo",
    ...
    await repository.insertDecision({
      id: "decision_post_completion_export_noise_1",
      runId: board.runId,
      tenantId: "tenant_123",
      actorUserId: "user_123",
      decisionKind: "proposal_denied",
      cardId: created.cardId,
      proposalId: "proposal_post_completion_export_noise_1",
      targetCardId: null,
      persona: "cto",
      deliverableType: "technical_review",
      policyReason: "scope_guardrail",
      resolution: null,
      decisionNote: "Should stay out of the persisted closed-board governance export bundle.",
      recommendationSummary: "Do not mutate the closed-board governance export bundle.",
      objectionSummary: "This late governance noise should not rewrite the frozen export surface.",
      createdAt: new Date().toISOString()
    });

    const hydratedBoard = await service.listBoardState({ authorization: "Bearer valid" });
    const refreshedExportAction = hydratedBoard.memoryBoundary.exportCandidates
      ?.find((entry) => entry.id === "governance_history_export")
      ?.exportActions?.find((entry) => entry.actionRoute === "governance-history-export");
    expect(refreshedExportAction).toBeTruthy();
    ...
    expect(secondExport.bundleRevision).toBe(firstExport.bundleRevision);
    expect(secondExport.content).toBe(firstExport.content);
    expect(secondExport.content).not.toContain("Open an extra technical review lane");
  });

  it("keeps the real package-bundle export bundle frozen after post-closure governance noise", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    const completedBoard = await service.listBoardState({ authorization: "Bearer valid" });
    const governanceCandidate = completedBoard.memoryBoundary.exportCandidates?.find(
      (entry) => entry.id === "governance_history_export"
    );
    const governanceExportAction = governanceCandidate?.exportActions?.find(
      (entry) => entry.actionRoute === "governance-history-export"
    );
    expect(governanceExportAction).toBeTruthy();
    const governanceExport = await service.exportGovernanceHistoryCandidate({
      authorization: "Bearer valid",
    ...
    const dependencyReadyBoard = await service.listBoardState({ authorization: "Bearer valid" });
    const packageExportAction = dependencyReadyBoard.memoryBoundary.exportCandidates
      ?.find((entry) => entry.id === "package_bundle_export")
      ?.exportActions?.find((entry) => entry.actionRoute === "package-bundle-export");
    expect(packageExportAction).toBeTruthy();
    ...
    await repository.insertProposal({
      id: "proposal_post_completion_package_noise_1",
      runId: board.runId,
      parentCardId: created.cardId,
      requestedByCardId: created.cardId,
      requestedByPersona: "cfo",
    ...
    await repository.insertDecision({
      id: "decision_post_completion_package_noise_1",
      runId: board.runId,
      tenantId: "tenant_123",
      actorUserId: "user_123",
      decisionKind: "proposal_denied",
      cardId: created.cardId,
      proposalId: "proposal_post_completion_package_noise_1",
      targetCardId: null,
      persona: "cto",
      deliverableType: "technical_review",
      policyReason: "scope_guardrail",
      resolution: null,
      decisionNote: "Should stay out of the persisted closed-board package export bundle.",
      recommendationSummary: "Do not mutate the closed-board package export bundle.",
      objectionSummary: "This late governance noise should not rewrite the frozen package export surface.",
      createdAt: new Date().toISOString()
    });

    const hydratedBoard = await service.listBoardState({ authorization: "Bearer valid" });
    const refreshedPackageExportAction = hydratedBoard.memoryBoundary.exportCandidates
      ?.find((entry) => entry.id === "package_bundle_export")
      ?.exportActions?.find((entry) => entry.actionRoute === "package-bundle-export");
    expect(refreshedPackageExportAction).toBeTruthy();

    const secondExport = await service.exportPackageBundleCandidate({
      authorization: "Bearer valid",
      runId: board.runId,
      candidateId: "package_bundle_export",
  ...
  it("keeps runtime continuity memory operational-only and out of tenant-owned export artifacts", async () => {
    const repository = createInMemoryHarnessRepository();
    const service = createHarnessBoardService({
      authenticate: vi.fn().mockResolvedValue({
        tenantId: "tenant_123",
    ...
    const board = await service.listBoardState({ authorization: "Bearer valid" });
    const runtimeOnlyContinuitySummary = "Runtime-only continuity summary that must never enter tenant-owned exports.";
    const created = await expectCreatedCard(service.createTopLevelChildCard({
      authorization: "Bearer valid",
      persona: "cfo",
      title: "Pressure-test the pricing lane",
    ...
    await service.advanceChildCard({
      ...
      cardId: created.cardId,
      state: "working",
      resumeSummary: runtimeOnlyContinuitySummary
    });
    await service.advanceChildCard({
      authorization: "Bearer valid",
      cardId: created.cardId,
    ...
    await repository.upsertExportDelivery({
      id: "delivery_governance_memory_boundary_phase19_1",
      runId: board.runId,
      tenantId: "tenant_123",
      workflowId: completedBoard.workflowId,
      packageId: completedBoard.packageId,
      ...
      redactionSummary: governanceExport.redactionSummary,
      attemptCount: 1,
      lastAttemptedAt: "2026-06-01T00:30:00.000Z",
      deliveredAt: "2026-06-01T00:30:01.000Z",
      writerKind: "obsidian_filesystem",
      deliveryReceipt: {
        primaryNotePath: governanceExport.placement.primaryNotePath,
        writtenFileCount: governanceExport.files.length
      ...
      updatedAt: "2026-06-01T00:30:01.000Z"
    });
    ...
    const packageExportAction = dependencyReadyBoard.memoryBoundary.exportCandidates
      ...
      ?.exportActions?.find((entry) => entry.actionRoute === "package-bundle-export");
    expect(packageExportAction).toBeTruthy();
    const packageExport = await service.exportPackageBundleCandidate({
      authorization: "Bearer valid",
      runId: board.runId,
      candidateId: "package_bundle_export",
      actionToken: packageExportAction!.actionHandle
