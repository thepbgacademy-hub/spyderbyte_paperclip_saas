...
- `npm run prove:live-soak-capacity -- --ssh-target deploy@<vps-ip> --sudo-password-file <path-to-sudo_deploy.txt> --queue-container wealth-factory-api-stage2 --docker-stats-out audit/<date>/live-soak-capacity-vX-docker-stats.jsonl --queue-snapshots-out audit/<date>/live-soak-capacity-vX-queue-snapshots.jsonl --proof-out audit/<date>/live-soak-capacity-vX-proof.json --interval-ms 10000 --cooldown-ms 15000 --ssh-timeout-ms 20000 -- --mode drain --order staggered --cycles 8 --cycle-interval-ms 1500 --queue-interval-ms 250 --timeout 240000 --post-success-observation-ms 120000 --lane primary:<tenant-a>:<user-a>:<workflow-a>:3 --lane secondary:<tenant-b>:<user-b>:<workflow-b>:2 --lane tertiary:<tenant-c>:<user-c>:<workflow-c>:2 --lane quaternary:<tenant-d>:<user-d>:<workflow-d>:1 --lane quinary:<tenant-e>:<user-e>:<workflow-e>:1 --lane senary:<tenant-f>:<user-f>:<workflow-f>:1`
...
- when running the long-soak collector against the staged VPS lane, prefer the repo-owned `prove:live-soak-capacity` wrapper so queue evidence comes from inside the staged API container instead of assuming Redis is reachable from the local workstation
- a healthy longer soak now means more than “all runs reached running”: the queue snapshot lane must remain reachable, and sustained-hotspot detection should only fire on repeated or long-lived threshold breaches rather than one-sample blips
- current six-lane soak read:
...
- current four-tenant follow-up read:
  ...
  - launch decision: four tenants per VPS is the strict launch upper cap, not a comfort target
  - anti-clustering rule: keep at least `120` seconds between tenant cron, heartbeat, onboarding, or manual launch-batch start windows until a later scheduler phase is explicitly approved
...
- current bounded-pod soak checkpoint on 2026-05-20:
  ...
  - treat this as a successful bounded-pod checkpoint for the six-client pod shape, not final proof of robustness; the next backlog item was longer soak duration, skewed bursts, and resource saturation behavior
  - current skewed-soak and saturation checkpoint on 2026-05-20:
    ...
    - this shorter skewed-burst checkpoint alone is not enough to clear the longer-soak risk
